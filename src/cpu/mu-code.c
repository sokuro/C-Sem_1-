...
void cpu_6502_lda_zp(){
  cycles = 3;
  //1st cycle
  cp_register(pcl, abrl); //programm counter low byte kopiert ins ABR low byte
  cp_register(pch, abrh);
  set_rw2read();
  access_memory();

  //2nd cycle
  cp_register(dbr, abrl); //get value from dbr & write in abrl (zero page)
  char zero[]="00000000"; ////set abrh to 0
...
void cpu_6502_lda_zpx (){
  ...
  //2nd cycle
  char localflags[]="00000000";
  alu(ALU_OP_ADD, dbr, idx, dbr, localflags); //lade zu dbr X und speichere in dbr
...
void cpu_6502_lda_izx(){
  ...
  alu(ALU_OP_ADD, dbr, idx, abrl, flags);
...
//  char tmp[8]; //tmp array, store low byte of adr
  char low[]="00000000";
/*  cp_register(dbr, tmp);*/
  cp_register(dbr, low);
  inc_pc();
...
//  cp_register(tmp, abrl); //absolute mode with low order bits
  cp_register(low, abrl);
  set_rw2read();
  access_memory();

  //4th cycle
  cp_register(dbr, acc); //write the value of dbr in acc
  zsflagging(flags, acc);
  inc_pc();
}
...
void cpu_6502_lda_abx (){
  ...
  //3th cycle
  alu(ALU_OP_ADD, tmp, idx, acc, flags);
...
void cpu_6502_lda_aby(){
  ...
  //3th cycle
  alu(ALU_OP_ADD, tmp, idy, acc, flags);
  cp_register(acc, abrl);
...
void cpu_6502_sta_zpx (){
  ...
  alu(ALU_OP_ADD, abrl, idx, abrl, flags);
  cp_register(acc, dbr);
  set_rw2write();
  access_memory();
  zsflagging(flags, acc);
...
void cpu_6502_sta_abs(){
  ...
  set_rw2read();
  access_memory();
  cp_register(dbr, low); //value speichert ins low byte
  ...
  set_rw2read();
  access_memory();
  cp_register(dbr, high); //value speichert ins high byte
  
  cp_register(low, abrl); //low byte speichert ins arbl
  cp_register(high, abrh);
  cp_register(acc, dbr); //Wert vom Acc speichert ins dbr
  set_rw2write();
  access_memory();
  inc_pc();
}
...
void cpu_6502_sta_abx (){
  ...
  alu(ALU_OP_ADD,low,idx,abrl,localflags); //add Wert von X und setzt localflags (wenn zu lb FF addiere..dann gibt es carry)
  alu(ALU_OP_ADD_WITH_CARRY,dbr,"00000000",abrh,localflags); //add 00000 weil ich nicht voraussehen kann, welchen Wert ich addiere
  cp_register(acc, dbr);
...
//  alu(ALU_OP_ADD_WITH_CARRY, dbr, "00000000",  abrh, localflags);
  ...
  alu(ALU_OP_ADD, abrl, idy, abrl, flags);
...
void cpu_6502_ldx_imm(){
  ...
  cp_register(dbr, idx); //WErt von dbr kopiere ich ins Index X
...
void cpu_6502_cmp_zp(){
  cycles = 3;
  char zero[REG_WIDTH+1]="00000000"; //xtra
...
void cpu_6502_inx_imp(){
  cycles = 2;
  inc_register(idx); //incr register with X/ sonst ich muss nichts verändern
}
